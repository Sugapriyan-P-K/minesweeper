...
public class puzzlegame {
    ...
    static boolean checkPlaces(int[][] board) {
        int i, j, number = 1, count = 0;
        for (i = 0; i < size; i++) {
            for (j = 0; j < size; j++) {
                if (board[i][j] == number++) {
                    ++count;
    ...
    static void doAction(char action) {
        switch (action) {
            case 'T':
                if (row - 1 < 0) {
                    System.out.println("Cannot move to top");
                } else {
            ...
            case 'B':
                if (row + 1 >= size) {
                    System.out.println("Cannot move to bottom");
                } else {
                    move(row, col, row + 1, col, currentBoard);
                    row = row + 1;
    ...
    static void start() {
        while (!(checkPlaces(currentBoard))) {
